...
describe('🔹 /api/v1/products', () => {
	...
	describe('PUT /:id', () => {
		...
		describe('DELETE /:id', () => {
			test('deleta produto (204)', async () => {
				const res = await withAuth(request.delete(`${BASE_URL}/${productId}`), token);
				expect(res.status).toBe(204);
			});
			test('retorna 400 para id inválido', async () => {
				const res = await withAuth(request.delete(`${BASE_URL}/0`), token);
				expect(res.status).toBe(400);
				expect(res.body.msg).toBe(MSG.INVALID_PARAM);
			});
			test('retorna 404 para id inexistente', async () => {
				const res = await withAuth(request.delete(`${BASE_URL}/000000000000000000000000`), token);
				expect(res.status).toBe(404);
				expect(res.body.msg).toBe(MSG.NOT_FOUND);
			});
			test('retorna 401 sem token', async () => {
